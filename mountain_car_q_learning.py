...
def run(episodes, epsilon, learning_rate, discount_factor, is_training):
    ...
    if is_training:
        ...
        f = open('static/uploads/mountain_car_QLearning.pkl', 'rb')
        q = pickle.load(f)
        f.close()
    ...
    if is_training:
        f = open('static/uploads/mountain_car_QLearning.pkl', 'wb')
        pickle.dump(q, f)
        f.close()
    ...
    if is_training:
        ...
        plt.legend()
        plt.title('Training Progress : Mountain Car - Q-Learning')
        plt.savefig('static/uploads/mountain_car_QLearning.png')
...
#
#     # run(1000, 1, 0.9, 0.9, is_training=True)
